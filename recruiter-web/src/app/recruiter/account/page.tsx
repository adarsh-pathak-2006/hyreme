...
export default function RecruiterAccountPage() {
  const { authStatus, logout, metrics, user } = useRecruiterState();
  const [isLoggingOut, setIsLoggingOut] = useState(false);

  const profile = user ?? {
    name: "Ritika Sharma",
    email: "ritika@hyreme.io",
  };
    ...
    <SectionCard
      ...
      description={
        authStatus === "authenticated"
          ? "Your recruiter account is connected to the live API session."
          : "Log in with the seeded recruiter account to connect this profile to the backend session."
      }
    >
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <article className="rounded-[1.75rem] border border-[var(--accent)]/20 bg-gradient-to-br from-[var(--accent-deep)] via-[var(--accent-strong)] to-[var(--accent)] p-6 text-white">
          ...
          <p className="mt-2 text-sm text-[color:rgba(255,255,255,0.82)]">Senior Talent Partner</p>
          ...
          <div className="mt-6 rounded-[1.5rem] border border-white/10 bg-white/8 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-[color:rgba(255,255,255,0.82)]">
              Recruiter operations for product, engineering, and customer
              success hiring.
            </p>
          </div>
          <button
